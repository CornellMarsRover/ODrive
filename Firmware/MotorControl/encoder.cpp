...
void Encoder::setup() {
    ...
    spi_task_.config = {
        ...
        .DataSize = (mode_ == 261)? SPI_DATASIZE_8BIT : SPI_DATASIZE_16BIT,//change to 8 bit mode CMR
        //cmr check
        .CLKPolarity = (mode_ == MODE_SPI_ABS_AEAT || mode_ == MODE_SPI_ABS_MA732) ? SPI_POLARITY_HIGH : SPI_POLARITY_LOW,
        .CLKPhase = (mode_ == 261)? SPI_PHASE_1EDGE:SPI_PHASE_2EDGE,//change to 1 edge CMR
        .NSS = SPI_NSS_SOFT,
...
bool Encoder::abs_spi_start_transaction() {
    if (mode_ & MODE_FLAG_ABS){
        if (Stm32SpiArbiter::acquire_task(&spi_task_)) {
            ...
            spi_task_.length = (mode_ = 261)? 4 : 1;//change to 4 for 4 8 bit messages for arm
