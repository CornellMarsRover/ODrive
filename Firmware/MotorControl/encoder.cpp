...
//         .DataSize = (mode_ == MODE_SPI_ABS_RLS)? SPI_DATASIZE_8BIT : SPI_DATASIZE_16BIT,//change to 8 bit mode CMR
//         //cmr check
//         .CLKPolarity = (mode_ == MODE_SPI_ABS_AEAT || mode_ == MODE_SPI_ABS_MA732) ? SPI_POLARITY_HIGH : SPI_POLARITY_LOW,
//         .CLKPhase = (mode_ == MODE_SPI_ABS_RLS) ? SPI_PHASE_1EDGE:SPI_PHASE_2EDGE,//change to 1 edge CMR
//         .NSS = SPI_NSS_SOFT,
//         .BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32,//SPI_BAUDRATEPRESCALER_16,//look at prescalar for clock CMR
...
//         abs_spi_dma_tx_[0] = 0x0000;//find startup command CMR
//     }
...
//             spi_task_.length = (mode_ = MODE_SPI_ABS_RLS)? 4 : 1;//change to 4 for 4 8 bit messages for arm
...
//             memcpy(&rawVal, abs_spi_dma_rx_, sizeof( uint32_t ));

//             uint16_t raw_pos = (( rawVal >> 16) & 0xFFFF ) ;
            

//             pos = ( raw_pos ) % (config_.cpr);//need to use max step/gear ratio when setting up

//             encoder_segment = (raw_pos / pos );
//         } break;
